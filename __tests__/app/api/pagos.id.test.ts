...
const getOrder = jest.requireMock('@/lib/lemonSqueezy').getOrder as jest.Mock
...
describe('GET /api/pagos/[id]', () => {
  ...
  it('actualiza monto y moneda cuando la orden sincroniza desde Lemon', async () => {
    const pagoRecord = {
      id: 'p3',
      estado: 'pendiente',
      checkout_id: 'chk_sync',
      order_id: 'ord_old',
      reserva_id: null,
      inscripcion_id: 22,
    }
    ...
    const pagosUpdateEqMock = jest.fn().mockResolvedValue({})
    const pagosUpdateMock = jest.fn().mockReturnValue({ eq: pagosUpdateEqMock })
    const inscripcionesUpdateEqMock = jest.fn().mockResolvedValue({})
    ...
    supabaseAdmin.from.mockImplementation((table: string) => {
      if (table === 'pagos') {
        return {
          ...
          update: pagosUpdateMock,
        }
      }
      if (table === 'inscripciones') {
        return { update: jest.fn().mockReturnValue({ eq: inscripcionesUpdateEqMock }) }
      }
      return {}
    })

    getCheckoutStatus.mockResolvedValue({ status: 'pending', orderId: 'ord_sync' })
    getOrder.mockResolvedValue({ status: 'paid', total: 1234, currency: 'USD' })
    findOrderByCheckoutId.mockResolvedValue(null)

    const response = await GET(new Request('https://example.com/api/pagos/p3'), { params: Promise.resolve({ id: 'p3' }) })

    expect(pagosUpdateMock).toHaveBeenCalledWith({ estado: 'pagado', order_id: 'ord_sync', monto_centavos: 1234, moneda: 'USD' })
    expect(pagosUpdateEqMock).toHaveBeenCalledWith('id', 'p3')
    expect(inscripcionesUpdateEqMock).toHaveBeenCalledWith('id', 22)
    expect(notifyPagoConfirmado).toHaveBeenCalledWith({
      previousEstado: 'pendiente',
      nextEstado: 'pagado',
      reservaId: null,
    ...
    expect(jsonSpy).toHaveBeenCalledWith({ pagoId: 'p3', estado: 'pagado', orderId: 'ord_sync', synced: true })
    expect(response).toEqual({ body: { pagoId: 'p3', estado: 'pagado', orderId: 'ord_sync', synced: true }, init: undefined })
  })

  it('devuelve errores de sincronización cuando fallan las consultas de orden y lookup', async () => {
    const pagoRecord = {
      id: 'p4',
      estado: 'pendiente',
      checkout_id: 'chk_error',
      order_id: null,
      reserva_id: 9,
      inscripcion_id: null,
    }
    ...
    supabaseAdmin.from.mockImplementation((table: string) => {
      ...
      if (table === 'reservas') {
        return { update: jest.fn().mockReturnValue({ eq: reservasUpdateEqMock }) }
      }
      return {}
    })

    getCheckoutStatus.mockResolvedValue({ status: 'pending', orderId: 'ord_err' })
    getOrder.mockRejectedValue(new Error('order_down'))
    findOrderByCheckoutId.mockRejectedValue(new Error('lookup_failed'))

    const response = await GET(new Request('https://example.com/api/pagos/p4'), { params: Promise.resolve({ id: 'p4' }) })

    expect(pagosUpdateMock).toHaveBeenCalledWith({ estado: 'pendiente', order_id: 'ord_err' })
    expect(pagosUpdateEqMock).toHaveBeenCalledWith('id', 'p4')
    expect(reservasUpdateEqMock).toHaveBeenCalledWith('id', 9)
    expect(notifyPagoConfirmado).toHaveBeenCalledWith({
      previousEstado: 'pendiente',
    ...
    expect(jsonSpy).toHaveBeenCalledWith({
      ...
      estado: 'pendiente',
      orderId: 'ord_err',
      orderSyncError: 'order_down',
      orderLookupError: 'lookup_failed',
      synced: true,
    })
